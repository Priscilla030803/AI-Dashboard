        ...
        with st.container():
            ...
            with col3:
                ...
                if not monthly_data.empty:
                    ...
                    st.plotly_chart(fig_visits_sales, use_container_width=True)
                else:
...
# --- SALES AGENT VIEW ---
    ...
    st.info(f"You have reached your annual sales target of **${personal_target:,.2f}**")

    col1, col2, col3 = st.columns(3)
    col1.metric(
        "Your Sales",
        f"${total_sales:,.2f}", delta=f"{((total_sales / personal_target) - 1) * 100:.1f}%" if personal_target > 0 else "N/A"
    )
